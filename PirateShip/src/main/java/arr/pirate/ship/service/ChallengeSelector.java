...
public class ChallengeSelector
{
    ...
    public Collection<Template> selectChallengeTemplates(User user)
    {
        Calendar now = Calendar.getInstance();
        TemplateSelection selection = selectionRepo.findValidSelection(user.getId(),
                                                                                now);
        if (selection == null)
        {
    ...
    private TemplateSelection generateSelection(User user, Calendar creation)
    {
        ...
        end.set(creation.get(Calendar.YEAR), creation.get(Calendar.MONTH), creation.get(Calendar.DAY_OF_MONTH));
        end.add(Calendar.DAY_OF_MONTH, 1);
        TemplateSelection cts = new TemplateSelectionBuilder().validFrom(creation).validTo(end).userId(user.getId())
                        .challenge1(indices.get(0)).challenge2(indices.get(1)).challenge3(indices.get(2)).build();
